# Solve MaxCut with TreeVQA for power grids
"""Power grid application for TreeVQA experiments."""
...
from .qaoa_TreeVQA.ieee14 import build_ieee14_graph_family


class PowerExperiment(TreeVQAApplication):
    """TreeVQA experiment implementation for MaxCut problems of power grids"""
    ...
    def result_dir_base(self) -> str:
        return f"{self.params['result_dir']}/power-grid/{self.params['graph_slice']}"
    ...
    def load_data(self) -> None:
        ...
        json_file = "ground-state/power-grid.json"
        with open(json_file, "r", encoding="utf-8") as f:
            energy_map = json.load(f)
            try:
                self.ref_values = [
                    energy_map[str(round(idx, 3))]["ground_energy"]
        ...
        graphs = build_ieee14_graph_family(self.selected_graph_indices)
        ...
        gamma_params = [
            ...
            for i, j in graphs[0].edge_list()
        ]
        beta_params = [
            Parameter(f"beta_{i}_{r}")
            for r in range(self.params["repetition"])
            for i in graphs[0].node_indexes()
        ]
