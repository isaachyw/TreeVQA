...
if __name__ == "__main__":
    from ieee14 import build_ieee14_unweighted_graph
    from graphs_utils import build_max_cut_paulis
    from red_qaoa import red_qaoa_exe
    from graphs_utils import rustworkx_to_networkx
    from circuit_utils import multi_angle_qaoa_circuit, assign_para_to_ma_ansatz
    ...
    ieee_graph = build_ieee14_unweighted_graph()
    original_ieee_graph = ieee_graph.copy()
    ieee_graph = rustworkx_to_networkx(ieee_graph)
    red_graph = red_qaoa_exe(ieee_graph)
    red_graph = rx.networkx_converter(red_graph)
    pauli_list = build_max_cut_paulis(red_graph)
    paulis = [p[0] for p in pauli_list]
    coeffs = [p[1] for p in pauli_list]
    pauli_ops = SparsePauliOp(data=paulis, coeffs=coeffs)
    num_qubits = pauli_ops.num_qubits
    gamma_params = [
        Parameter(f"gamma_{i}_{j}_{r}")
        for r in range(2)
        for i, j in red_graph.edge_list()
    ]
    beta_params = [
        Parameter(f"beta_{i}_{r}") for r in range(2) for i in red_graph.node_indexes()
    ]
    original_gamma_params = [
        Parameter(f"gamma_{i}_{j}_{r}")
        for r in range(2)
        for i, j in original_ieee_graph.edge_list()
    ]
    original_beta_params = [
        Parameter(f"beta_{i}_{r}")
        for r in range(2)
        for i in original_ieee_graph.node_indexes()
    ]
    qc = QAOAAnsatz(cost_operator=pauli_ops, reps=2).decompose().decompose()
    ma_ansatz = multi_angle_qaoa_circuit(
        gamma_params, beta_params, num_qubits, red_graph, 2
    )
    original_ma_ansatz = multi_angle_qaoa_circuit(
        original_gamma_params, original_beta_params, 12, original_ieee_graph, 2
    )
    original_x0 = assign_para_to_ma_ansatz(original_ma_ansatz, qc.parameters)
    ...
    def _cost_function(params, circuit, cost_hamiltonian, esti):
        """
    ...
    # maqa x is [0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 0.98467873, -0.00867152, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082, 1.25621434, 0.29949082]
